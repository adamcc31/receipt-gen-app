/**
 * Receipt Template - Nenkin Type
 * 
 * Professional receipt template for Nenkin NORMAL and Nenkin SPEED transactions.
...
export interface NenkinReceiptData {
    receiptNumber: string;
    date: string;
    clientName: string;
    type: 'NENKIN_NORMAL' | 'NENKIN_SPEED';  // Explicit type
    nominalYen: Decimal | string;
    regionalTaxYen?: Decimal | string;
    adminFeePercentage: Decimal | string;
    adminFeeAmount: Decimal | string;
    taxFixed: Decimal | string;
    speedServiceFee?: Decimal | string;      // Optional - only for NENKIN_SPEED
    ...
    shippingFeeIdr?: Decimal | string;
    finalNominalIdr: Decimal | string;
    note?: string;                           // Optional footer note
}

interface ReceiptNenkinProps {
    data: NenkinReceiptData;
    density?: 'comfortable' | 'compact';
}

export default function ReceiptNenkin({ data, density = 'compact' }: ReceiptNenkinProps) {
    const isCompact = density === 'compact';
    const adminPercentDisplay = new Decimal(data.adminFeePercentage).mul(100).toString();
    const hasSpeedService = data.type === 'NENKIN_SPEED' && data.speedServiceFee;
    const hasAdditionalCost = data.additionalCostAmount && new Decimal(data.additionalCostAmount).greaterThan(0);
    const hasRegionalTax = data.regionalTaxYen && new Decimal(data.regionalTaxYen).greaterThan(0);
    const hasShippingFee = data.shippingFeeIdr && new Decimal(data.shippingFeeIdr).greaterThan(0);
    const hasNote = data.note && data.note.trim() !== '';
    const accent =
        data.type === 'NENKIN_SPEED'
            ? {
                primary: '#2563EB',
            }
            : {
        ...
        >
            ...
            >
                <div style={styles.logoArea}>
                    <div style={{ ...styles.companyName, color: '#FFFFFF' }}>EXATA INDONESIA</div>
                    <div style={{ ...styles.companySubtitle, color: '#FFFFFF' }}>Financial Services</div>
                </div>
            ...
            <div style={styles.tableContainer}>
                <table style={styles.table}>
                    ...
                    <tbody>
                        ...
                        {hasRegionalTax && (
                            <tr>
                                <td style={tdStyle}>Pajak Daerah (JPY)</td>
                                <td style={{ ...tdStyle, textAlign: 'right', color: '#DC2626' }}>
                                    - {formatJpy(data.regionalTaxYen!)}
                                </td>
                            </tr>
                        )}
                        <tr>
                        ...
                        <tr>
                            ...
                            <td style={{ ...tdStyle, textAlign: 'right', color: '#DC2626' }}>
                                - {formatJpy(data.taxFixed)}
                            </td>
                        </tr>

                        {/* Conditional: Speed Service Row - Only for NENKIN_SPEED */}
                        {hasSpeedService && (
                            <tr>
                                <td style={tdStyle}>Biaya Speed Service</td>
                                <td style={{ ...tdStyle, textAlign: 'right', color: '#DC2626' }}>
                                    - {formatJpy(data.speedServiceFee!)}
                                </td>
                            </tr>
                        )}
